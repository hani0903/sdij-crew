...
function WeekDayCell({
    ...
    return (
        <li
            ...
            className={`
        relative flex flex-col gap-2 p-4 text-12 font-semibold items-center flex-1
        ${isWeekend ? 'text-[#94A3B8] cursor-default' : 'cursor-pointer'}
        ${isSelected ? 'text-point' : !isWeekend && 'text-[#0F172A]'}
      `}
        >
            {isToday && <span className="absolute bottom-2 left-1/2 -translate-x-1/2 size-1 bg-red-500 rounded-full" />}
...
export default function WeekDayBar({ selectedWeekday, onSelectWeekday }: WeekDayBarProps) {
    return (
        <ul className="w-full flex items-center justify-center transition-all duration-200 border-b-2 border-[#E2E8F0] px-2">
            {getWeekDays().map((weekday) => (
                <WeekDayCell
                    ...
                    isSelected={weekday.weekday === selectedWeekday}
                    onClick={() => onSelectWeekday(weekday.weekday)}
                />
            ))}
        </ul>
    );
